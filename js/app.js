document.querySelector('#dark-mode-toggle').addEventListener('click', () => {
    ...
    document.querySelector('meta[name="theme-color"').setAttribute('content', isDarkMode ? '#404040' : '#fff');
});
...
let su = undefined; //change variable name
let su_answer = undefined; //change variable name
...
const initSudoku = () => {
    ...
    clearSudoku();
    resetBg();
    // generate numbers in puzzle
    su = sudokuGen(level); // change name for variables w/ su
...
document.querySelector('#btn-play').addEventListener('click', () => {
    if (name_input.value.trim().length > 0) {
        initSudoku();
        startGame();
        // use grave accent (`) to print value of variable, apostrophe (') prints string
    } else {
...
const init = () => {
    ...
    document.querySelector('meta[name="theme-color"').setAttribute('content', darkmode ? '#404040' : '#fff');

    const game = getGameInfo();

    document.querySelector('#btn-continue').style.display = game ? 'grind' : 'none';
// - - - - - - - - - -
